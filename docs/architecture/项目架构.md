...
> 更新时间：2026-07-20
> 范围：基于当前仓库 `kubeoncall` 的前端、后端、配置和部署资产整理。
...
当前仓库采用前后端分离的单仓库结构。`backend/` 是 Spring Boot 单体后端，主要承担：
...
`frontend/` 是独立静态 Web Console，用于调试服务状态、问答、审批和原始 API。它拥有独立构建、契约测试和 Nginx 镜像，不再由 Spring Boot 打包静态资源。根目录保留 Docker Compose、部署资源、脚本、正式文档和 CI。
...
- 一次问答执行的 `GraphState`
- 一次告警接入的 `AlarmIngress` 与处理事件 `NormalizedAlarmEvent`
...
- Java 17
- Spring Boot 3.3.4
- Maven Wrapper：位于 `backend/`，统一后端验证命令为 `cd backend && ./mvnw verify`
- Node.js 20+：执行前端契约测试与静态资源构建
...
平台级配置由 `KubeOnCallProperties` 作为绑定根对象，并按配置域拆分为独立类型；仍由 `kubeoncall.*` 绑定且保持既有调用 API。当前主要配置域包括：
...
- `kubeoncall.cors.*`：独立前端直接访问后端时的显式来源白名单
...
- `backend/src/main/resources/skills/payment-oom-triage/SKILL.md`
...
| `service` | 应用级编排、审计、metrics；`AskContextLifecycle` 负责问答会话、记忆、Skill 与上下文生命周期，`GraphAuditMetadataFactory` 负责 GraphState 审计字段构造 |
| `agent` | Planner / Verifier / Executor 多 Agent 链路；`PlannerContextAssembler` 负责 Planner 的请求上下文、知识与 Skill 上下文装配，`PlannerRuleEngine` 负责 Planner 规则推断与任务构造，`ExecutorPlanFactory` 负责 Executor 参数与执行计划装配 |
| `alarm` | 告警标准化、领域模型、策略引擎、fingerprint、active/silence 状态 |
| `workflow` | 告警工作流模板、事件准备、节点执行、告警记忆、降级、审计与依赖控制；`AlarmEventPreparationService`、`AlertWorkflowRunner`、`AlertWorkflowMemory`、`AlarmNodeNoiseSuppression`、`AlarmWorkflowEscalation` 与 `AlarmWorkflowAuditRecorder` 分别负责事件治理、节点运行、记忆生命周期、节点噪声抑制、升级判定和告警审计 |
...
| `notification` | 平台无关消息与路由、逐目的地投递表、Outbox 重试/死信/重放、飞书与钉钉单向群机器人；真实租户验收仍需单独执行 |
| `domain` | graph / task / audit / rag / approval / legacy alarm 等领域状态对象 |
| `common` | 配置与通用异常 |

`ArchitectureRulesTest` 以 ArchUnit 保护两条基础依赖方向：核心业务域不得反向依赖 `web`；`web` 不得直接引用 Redis、Elasticsearch 或 MinIO 客户端。现有 Controller 到领域仓储的历史依赖将在后续应用服务迁移中逐步收敛。
...
flowchart TD
    ...
    ASK --> ASKCTX[AskContextLifecycle]
    ASKCTX --> SESSION[SessionStore]
    ASKCTX --> MINJ[MemoryInjector]
    ASKCTX --> SKILL[SkillActivationService]
    ...
    ALARM --> PREP[AlarmEventPreparationService]
    ALARM --> ALARMMEM[AlertWorkflowMemory]
    PREP --> POLICY[AlarmPolicyEngine]
    PREP --> ACTIVE[ActiveAlarmStore]
    ALARM --> RUNNER[AlertWorkflowRunner]
    ALARM --> NOISE[AlarmNodeNoiseSuppression]
    ALARM --> ESCALATE[AlarmWorkflowEscalation]
    ALARM --> ALARMAUDIT[AlarmWorkflowAuditRecorder]
    RUNNER --> WFT[WorkflowTemplateRegistry]
...
sequenceDiagram
    ...
    participant A as AlarmWorkflowAuditRecorder
    ...
    W->>A: record workflow audit
    A->>E: record alarm execution
    W-->>C: List<NodeResult>
```
...
- `integration-test` profile 通过 Failsafe 执行 `ElasticsearchKnowledgeE2EIT`、`RedisSkillStateStoreIT`、`MinioKnowledgeObjectStorageIT` 与 `ToolHttpClientIT`；后者使用进程内可控 HTTP 服务，不依赖 Docker
...
- 持续验证前后端 Dockerfile、Helm Chart、GitHub Actions 等交付资产的镜像构建、lint 与部署路径
...
1. 根目录 `README.md`、`docker-compose.yml`、`Makefile`
2. `backend/pom.xml`、`backend/mvnw`、`backend/src/main/resources/application.yml`
3. `frontend/src/index.html`、`frontend/src/app.js`
4. `KubeOnCallProperties`
5. `AskController`、`AskService`
6. `PlannerAgent`、`VerifierAgent`、`ExecutorAgent`
7. `GraphState`、`ApprovalService`、`GraphStateStore`
8. `AlarmController`、`AlarmNormalizer`
9. `AlarmPolicyEngine`、`YamlAlarmPolicyRepository`、`alarm-policies.yml`
10. `AlertWorkflowService`、`WorkflowTemplateRegistry`、`AlertWorkflowFactory`
11. `workflow.node.*`
12. `KnowledgeIngestService`、`KnowledgeIngestionFacade`、`KnowledgeRetrievalFacade`
13. `HybridRetrievalService`、`EmbeddingService`、`VectorRetriever`、`RerankService`
14. `MemoryService`、`DefaultMemoryInjector`、`HeuristicMemoryExtractor`、`AlertMemoryService`
15. `SkillRegistry`、`SkillMatcher`、`SkillActivationService`
16. `ToolExecutor`、`AgentToolCatalog`、各具体执行器
17. `ExecutionAuditService`、`KubeOnCallMetricsService`
...
- `backend/src/main/resources/alarm-policies.yml`
- `backend/src/main/resources/skills/payment-oom-triage/SKILL.md`
- `backend/src/main/java/com/kubeoncall/common/config/KubeOnCallProperties.java`

### Web 层

- `backend/src/main/java/com/kubeoncall/web/AskController.java`
- `backend/src/main/java/com/kubeoncall/web/AlarmController.java`
- `backend/src/main/java/com/kubeoncall/web/ApprovalController.java`
- `backend/src/main/java/com/kubeoncall/web/KnowledgeController.java`
- `backend/src/main/java/com/kubeoncall/web/MemoryController.java`
- `backend/src/main/java/com/kubeoncall/web/ToolController.java`
- `backend/src/main/java/com/kubeoncall/web/StatsController.java`
- `backend/src/main/java/com/kubeoncall/web/StatusController.java`

### 应用服务

- `backend/src/main/java/com/kubeoncall/service/AskService.java`
- `backend/src/main/java/com/kubeoncall/service/ExecutionAuditService.java`
- `backend/src/main/java/com/kubeoncall/service/KubeOnCallMetricsService.java`

### Agent 与审批

- `backend/src/main/java/com/kubeoncall/agent/planner/PlannerAgent.java`
- `backend/src/main/java/com/kubeoncall/agent/verifier/VerifierAgent.java`
- `backend/src/main/java/com/kubeoncall/agent/executor/ExecutorAgent.java`
- `backend/src/main/java/com/kubeoncall/agent/base/ReActAgent.java`
- `backend/src/main/java/com/kubeoncall/agent/base/PipelineAgent.java`
- `backend/src/main/java/com/kubeoncall/agent/composer/ResponseComposer.java`
- `backend/src/main/java/com/kubeoncall/approval/ApprovalService.java`
- `backend/src/main/java/com/kubeoncall/state/GraphStateStore.java`

### 告警治理

- `backend/src/main/java/com/kubeoncall/alarm/ingest/AlarmNormalizer.java`
- `backend/src/main/java/com/kubeoncall/alarm/policy/AlarmFingerprintService.java`
- `backend/src/main/java/com/kubeoncall/alarm/policy/AlarmPolicyEngine.java`
- `backend/src/main/java/com/kubeoncall/alarm/policy/YamlAlarmPolicyRepository.java`
- `backend/src/main/java/com/kubeoncall/alarm/state/ActiveAlarmStore.java`
- `backend/src/main/java/com/kubeoncall/alarm/state/AlarmSilenceApprovalStore.java`
- `backend/src/main/java/com/kubeoncall/workflow/AlertWorkflowService.java`
- `backend/src/main/java/com/kubeoncall/workflow/AlarmWorkflowAuditRecorder.java`
- `backend/src/main/java/com/kubeoncall/workflow/WorkflowTemplateRegistry.java`
- `backend/src/main/java/com/kubeoncall/workflow/AlertWorkflowFactory.java`
- `backend/src/main/java/com/kubeoncall/workflow/WorkflowNodeExecutor.java`
- `backend/src/main/java/com/kubeoncall/workflow/node/SilenceNode.java`

### RAG、记忆与 Skill

- `backend/src/main/java/com/kubeoncall/rag/KnowledgeIngestService.java`
- `backend/src/main/java/com/kubeoncall/rag/KnowledgeIngestionFacade.java`
- `backend/src/main/java/com/kubeoncall/rag/KnowledgeRetrievalFacade.java`
- `backend/src/main/java/com/kubeoncall/rag/HybridRetrievalService.java`
- `backend/src/main/java/com/kubeoncall/rag/EmbeddingService.java`
- `backend/src/main/java/com/kubeoncall/rag/VectorRetriever.java`
- `backend/src/main/java/com/kubeoncall/rag/RerankService.java`
- `backend/src/main/java/com/kubeoncall/memory/MemoryService.java`
- `backend/src/main/java/com/kubeoncall/memory/DefaultMemoryInjector.java`
- `backend/src/main/java/com/kubeoncall/memory/HeuristicMemoryExtractor.java`
- `backend/src/main/java/com/kubeoncall/memory/AlertMemoryService.java`
- `backend/src/main/java/com/kubeoncall/skill/SkillRegistry.java`
- `backend/src/main/java/com/kubeoncall/skill/SkillMatcher.java`
- `backend/src/main/java/com/kubeoncall/skill/SkillActivationService.java`

### 存储与工具

- `backend/src/main/java/com/kubeoncall/storage/KnowledgeObjectStorageService.java`
- `backend/src/main/java/com/kubeoncall/storage/MinioDocumentStore.java`
- `backend/src/main/java/com/kubeoncall/tool/ToolExecutor.java`
- `backend/src/main/java/com/kubeoncall/tool/AgentToolCatalog.java`
- `backend/src/main/java/com/kubeoncall/tool/k8s/KubernetesToolExecutor.java`
- `backend/src/main/java/com/kubeoncall/tool/monitoring/PrometheusToolExecutor.java`
- `backend/src/main/java/com/kubeoncall/tool/alerting/AlertmanagerToolExecutor.java`
