...
class ActiveAlarmBackfillRunnerTest {
    ...
    void dryRunScansButDoesNotProject() {
        ...
        stubScan(redis, "alarm-active:fp_dry");
        stubLease(redis);
    ...
    void missingStateIsSkipped() {
        StringRedisTemplate redis = mock(StringRedisTemplate.class);
        stubScan(redis, "alarm-active:fp_missing");
        stubLease(redis);
        ActiveAlarmStore store = mock(ActiveAlarmStore.class);
    ...
    @Test
    void applyContinuesFromPersistedCursorAndAdvancesOnlyAfterThePage() {
        StringRedisTemplate redis = mock(StringRedisTemplate.class);
        stubScan(redis, "alarm-active:fp_checkpoint");
        stubLease(redis);
        ActiveAlarmStore store = mock(ActiveAlarmStore.class);
        ActiveAlarmState state = new ActiveAlarmState(
                "fp_checkpoint",
                ...
                Instant.parse("2026-07-20T01:10:00Z"),
                1);
        when(store.find("fp_checkpoint")).thenReturn(Optional.of(state));
        AlarmIncidentProjection projection = mock(AlarmIncidentProjection.class);
        when(projection.isAvailable()).thenReturn(true);
        MigrationLedgerRepository ledger = mock(MigrationLedgerRepository.class);
        when(ledger.loadScanCheckpoint("active-alarm"))
        ...
        MigrationLedgerRepository.MigrationWriteFence fence =
                new MigrationLedgerRepository.MigrationWriteFence("active-alarm", "owner", 1L);
        when(ledger.claimWriteFence(eq("active-alarm"), any())).thenReturn(fence);
        ...
        ActiveAlarmBackfillRunner.BackfillResult result = runner.run(false, "req_checkpoint");

        assertThat(result.checkpoint()).isEqualTo("0");
        verify(projection).projectBackfill(state);
        verify(ledger).advanceScanCheckpointFenced(fence, "active-alarm", "0", true);
        verify(ledger).loadScanCheckpoint("active-alarm");
    }
