...
export function UserManagementPage() {
  ...
  const disableMutation = useMutation({
    mutationFn: (user: UserView) =>
      api.post(
        `/api/v1/users/${encodeURIComponent(user.id)}/disable`,
        undefined,
        revisionOptions(user),
      ),
  ...
  const enableMutation = useMutation({
    mutationFn: (user: UserView) =>
      api.post(
        `/api/v1/users/${encodeURIComponent(user.id)}/enable`,
        undefined,
        revisionOptions(user),
      ),
    onSuccess: invalidate,
  ...
  const revokeMutation = useMutation({
    mutationFn: (user: UserView) =>
      api.post(
        `/api/v1/users/${encodeURIComponent(user.id)}/session-revocations`,
        undefined,
        revisionOptions(user),
      ),
    onSuccess: invalidate,
                  ...
                  <div className="koc-pagination__actions">
                    {user.status === 'ACTIVE' ? (
                      <Button
                        ...
                        onClick={() => disableMutation.mutate(user)}
                      >
                        禁用
                      </Button>
                    ) : (
      ...
      <div className="koc-dialog__panel">
        ...
        <section className="koc-detail__summary">
              ...
              <Button
                ...
                onClick={() =>
                  void run(
                    () =>
                      api.delete(
                        `${basePath}/roles/${encodeURIComponent(assignedRole)}`,
                        undefined,
                        revisionOptions(user),
                      ),
                    'role',
          ...
          <div className="koc-dialog__actions">
            ...
            <Button
              ...
              onClick={() =>
                void run(
                  () => api.post(`${basePath}/roles`, { role }, revisionOptions(user)),
                  'role',
                )
              }
            >
        ...
        <section className="koc-detail__summary">
          ...
          <Button
            ...
            onClick={() =>
              void run(
                () => api.post(`${basePath}/password`, { password }, revisionOptions(user)),
                'password',
              )
            }
          >
