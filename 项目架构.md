...
当前仓库未包含独立前端工程；后端交付资产包括根目录 `Dockerfile`、`deploy/helm/kubeoncall` Helm Chart、单文件 Kubernetes 清单以及 GitHub Actions CI。`docker-compose.yml` 提供本地 Redis / Elasticsearch / MinIO 依赖。
...
- Java 17
- Spring Boot 3.3.4
- Maven Wrapper：统一本地与 CI 验证命令为 `./mvnw verify`
...
- 持续验证现有 Dockerfile、Helm Chart、GitHub Actions 等交付资产的镜像构建、lint 与部署路径
...
- `src/main/java/com/kubeoncall/workflow/AlarmWorkflowAuditRecorder.java`
