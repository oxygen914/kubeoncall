...
class HttpEmbeddingClientTest {
    ...
    void shouldUseOpenAiCompatibleRequestAndResponseShape() {
        ...
        when(httpClient.post(eq("http://embedding/v1/embeddings"), anyMap(), anyInt(), anyMap()))
                .thenReturn(Map.of(
                        "status", "success",
        ...
        verify(httpClient).post(eq("http://embedding/v1/embeddings"), bodyCaptor.capture(), anyInt(), anyMap());
