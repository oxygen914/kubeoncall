...
class AlarmEscalationServiceTest {
    ...
    void shouldDeliverEscalationToNotificationAndIncidentSystems() {
        ...
        when(alertmanager.execute(eq("sendAlertEvent"), any())).thenReturn(Map.of("status", "success", "httpStatus", 200));
        when(incident.execute(eq("escalateIncident"), any())).thenReturn(Map.of("status", "success", "httpStatus", 200));
    ...
    private static NormalizedAlarmEvent event() {
        return new NormalizedAlarmEvent(
                "alarm-1", "fp-escalate", "HostHighCpuUsageP0", "prometheus", "critical", AlarmSeverity.P0,
                null, "node-a", "cluster-a", "prod", "infra", "host.cpu.usage_percent",
                91.0, 85.0, "%", "5m", Map.of(), Map.of(), "runbook-host-cpu-high", null,
                Instant.now(), "cpu critical", Map.of());
    }

    private static AlarmEvaluationResult evaluation() {
        return new AlarmEvaluationResult(
                true, null, "host-high-cpu-p0", AlarmSeverity.P0, 85.0, "runbook-host-cpu-high",
                "cpu > 85", "10m", "host-resource", "matched", List.of());
    }
}
