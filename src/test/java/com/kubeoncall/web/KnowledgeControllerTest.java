...
class KnowledgeControllerTest {
    ...
    void shouldAuditAndMeasureKnowledgeIngest() {
        ...
        KnowledgeDocument document = new KnowledgeDocument(
                "doc-1", "CPU SOP", "check cpu", "manual", Map.of(), Instant.now());
        when(knowledgeIngestService.ingest("CPU SOP", "check cpu", "manual", Map.of()))
                .thenReturn(document);
        KnowledgeController controller = new KnowledgeController(
                knowledgeIngestService, runbookImportService, mock(KnowledgeIndexAdmin.class),
                auditService, metricsService);

        KnowledgeDocument result = controller.ingest(
                new KnowledgeIngestRequest("CPU SOP", "check cpu", "manual", Map.of()));

        assertTrue("doc-1".equals(result.id()));
        verify(metricsService).recordKnowledge("ingest", "SUCCESS", 1);
        verify(auditService).recordKnowledgeOperation(
                eq("ingest"), eq("SUCCESS"), any(), any(Instant.class), any(Map.class));
    }
}
