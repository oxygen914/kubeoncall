...
public class ApprovalController {
    ...
    public AskResponse decide(@PathVariable String executionId,
                              @Valid @RequestBody ApprovalDecisionRequest request) {
        ApprovalDecision decision = ApprovalDecision.valueOf(request.decision().toUpperCase());
        AskService.ApprovalExecutionResult result = askService.decideAndResume(
                executionId,
                decision,
                request.comment(),
                request.decidedBy()
        );
        return new AskResponse(result.executionId(), result.status(), result.message());
    }
}
