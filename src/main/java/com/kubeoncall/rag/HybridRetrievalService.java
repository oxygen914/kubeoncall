...
public class HybridRetrievalService {
    ...
    public HybridRetrievalService(
        ...
        this.vectorRetrievers = List.copyOf(vectorRetrievers);
    ...
    public RetrievalTrace retrieveWithTrace(RetrievalRequest request) {
        ...
        int lexicalCandidateTopN =
                Math.min(200, Math.max(policyCandidateTopN, properties.getRag().getLexicalCandidateTopN()));
        int vectorCandidateTopN =
                Math.min(200, Math.max(policyCandidateTopN, properties.getRag().getVectorCandidateTopN()));
        ...
        long lexicalLatencyMs =
                Duration.between(lexicalStartedAt, Instant.now()).toMillis();
        List<KnowledgeDocument> lexicalCandidates =
                lexicalHits.stream().map(RetrievalHit::document).toList();
        ...
        List<KnowledgeDocument> vectorCandidates =
                vectorHits.stream().map(RetrievalHit::document).toList();
        ...
        Map<String, Double> lexicalRanks =
                rankScores(lexicalHits, properties.getRag().getRrfK());
        Map<String, Double> vectorRanks =
                rankScores(vectorHits, properties.getRag().getRrfK());
        ...
        List<KnowledgeDocument> fused = all.values().stream()
                .sorted(Comparator.comparingDouble((KnowledgeDocument doc) ->
                                lexicalRanks.getOrDefault(doc.id(), 0.0) + vectorRanks.getOrDefault(doc.id(), 0.0))
                        .reversed()
        ...
        diagnostics.put(
                "queryLength",
                request.question() == null ? 0 : request.question().trim().length());
        ...
        diagnostics.put(
                "fusedDocumentIds", fused.stream().map(KnowledgeDocument::id).toList());
        diagnostics.put(
                "lexicalDocumentIds",
                lexicalCandidates.stream().map(KnowledgeDocument::id).toList());
        diagnostics.put(
                "vectorDocumentIds",
                vectorCandidates.stream().map(KnowledgeDocument::id).toList());
    ...
    private VectorSearchResult searchVectorCandidates(RetrievalRequest request, int candidateSize) {
        for (VectorRetriever retriever : vectorRetrievers) {
            ...
            return new VectorSearchResult(hits, retriever.source());
        }
    ...
    private List<RetrievalHit> fusedHits(
            List<KnowledgeDocument> documents, Map<String, Double> lexicalRanks, Map<String, Double> vectorRanks) {
        List<RetrievalHit> hits = new ArrayList<>();
        int rank = 1;
        for (KnowledgeDocument document : documents) {
            double score = lexicalRanks.getOrDefault(document.id(), 0.0) + vectorRanks.getOrDefault(document.id(), 0.0);
    ...
    private void recordMetrics(
            RetrieveMethod method, String vectorSource, boolean vectorFallback, long resultCount, long latencyMs) {
    ...
    public record RetrievalTrace(
            List<KnowledgeDocument> documents, List<String> reasons, Map<String, Object> diagnostics) {}
