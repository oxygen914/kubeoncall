...
func TestBuilderEmitsHardenedJobAndRejectsUnsafeInputs(t *testing.T) {
	limits := Limits{CPUMilli: 250, MemoryMiB: 256, EphemeralMiB: 256, TimeoutSeconds: 120, TTLSeconds: 3600}
	builder := Builder{Namespace: "kubeoncall-sandbox", Tools: map[string]Tool{"pod-inspect:v1": {ID: "pod-inspect", Version: "v1", Image: "registry/pod-inspect@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Entrypoint: []string{"/tool"}, Runtime: RuntimeFixedDiagnostic, NetworkEgressPolicy: NetworkEgressDNSAndArtifactChannel, Limits: limits}}, Ceiling: Limits{CPUMilli: 500, MemoryMiB: 512, EphemeralMiB: 512, TimeoutSeconds: 300, TTLSeconds: 3600}}
	...
	if spec.Limits != limits || spec.Environment["SANDBOX_NETWORK_EGRESS"] != "DNS_AND_ARTIFACT_CHANNEL" {
		t.Fatalf("tool-specific policy was not enforced: %#v", spec)
	}
...
func TestBuilderLimitsGeneratedCodeToFixedRuntimeAndOutputContract(t *testing.T) {
	builder := Builder{Namespace: "kubeoncall-sandbox", Tools: map[string]Tool{
		"generated-python:v1": {
			...
			Entrypoint: []string{"/usr/local/bin/koc-run-python"}, Runtime: RuntimeGeneratedPython, NetworkEgressPolicy: NetworkEgressDenyAll,
			Limits: Limits{CPUMilli: 250, MemoryMiB: 256, EphemeralMiB: 256, TimeoutSeconds: 60, TTLSeconds: 3600}, MaxProcesses: 32, OutputMaxBytes: 1024 * 1024,
		},
	...
	builder.Tools["generated-python:v1"] = Tool{ID: "generated-python", Version: "v1", Image: "registry/generated-python@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd", Entrypoint: []string{"/tool"}, Runtime: RuntimeGeneratedPython, NetworkEgressPolicy: NetworkEgressDenyAll, Limits: Limits{CPUMilli: 250, MemoryMiB: 256, EphemeralMiB: 256, TimeoutSeconds: 60, TTLSeconds: 3600}, MaxProcesses: 33, OutputMaxBytes: 1}
...
func TestBuilderRejectsToolLimitsAboveControllerCeiling(t *testing.T) {
	builder := Builder{
		Namespace: "kubeoncall-sandbox",
		Tools: map[string]Tool{"pod-inspect:v1": {
			ID: "pod-inspect", Version: "v1",
			Image:      "registry/pod-inspect@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
			Entrypoint: []string{"/tool"}, Runtime: RuntimeFixedDiagnostic,
			NetworkEgressPolicy: NetworkEgressDenyAll,
			Limits:              Limits{CPUMilli: 750, MemoryMiB: 256, EphemeralMiB: 256, TimeoutSeconds: 60, TTLSeconds: 3600},
		}},
		Ceiling: Limits{CPUMilli: 500, MemoryMiB: 512, EphemeralMiB: 512, TimeoutSeconds: 120, TTLSeconds: 3600},
	}
	if _, err := builder.Build(Request{RunID: "sbx_abcd", ToolID: "pod-inspect", ToolVersion: "v1", InputArtifactURI: "minio://x"}); err == nil {
		t.Fatal("expected per-tool limit above controller ceiling to be rejected")
	}
}
